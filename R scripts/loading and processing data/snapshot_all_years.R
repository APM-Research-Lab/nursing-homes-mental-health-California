...
## calculate the number of people with SMI 
  ...
  all_residents_final <- all_residents_final %>% 
    ...
    mutate(SMI = ifelse((i5900_mnc_dprsn_cd == 1 | i5950_psychtc_cd == 1 | i6000_schzoprnia_cd == 1), 1, 0),
           ...
           # and/or psychotic disorder AND does NOT have dementia or alzheimers 
           ...
           # just dementia no SMI
           dementia_NO_SMI = ifelse((SMI == 0 & (i4200_alzhmr_cd == 1 | i4800_dmnt_cd == 1)), 1, 0),
           # SMI and dementia
           dementia_SMI = ifelse((SMI == 1 & (i4200_alzhmr_cd == 1 | i4800_dmnt_cd == 1)), 1, 0))
  ...
  # add new column indicating if resident is under 65 and one if they are SMI resident and received therapy
  all_residents_final <- all_residents_final %>% 
    mutate(age_under_65 = ifelse(c_rsdnt_age_num < 65, 1, 0)) %>%
    mutate(SMI_therapy = ifelse((SMI == 1 & o0400e1_psych_thrpy_min_num > 0), 1, 0))
    
  
  ## export all residents snapshot as csv 
  write.csv(all_residents_final, paste0('final_processed_snapshot/', year, '_CA_all_residents_snapshot.csv'))
  ...
  all_residents_final_grouped_summed <- all_residents_final %>%   
    ...
    summarise(PASRR_SMI = sum(a1510a_srus_mentl_ill_cd, na.rm = TRUE),
              ...
              #sum people with dementia and no SMI 
              CALC_dementia_NO_SMI = sum(dementia_NO_SMI, na.rm = TRUE),
              #sum people with dementia and SMI 
              CALC_dementia_SMI = sum(dementia_SMI, na.rm = TRUE),
              ...
              # calculate % of residents with our definition of SMI who have dementia
              SMI_dementia_per_resident = CALC_dementia_SMI/total_residents_dec_31,
              # calculate % of residents with dementia and no SMI
              dementia_NO_SMI_per_resident = CALC_dementia_NO_SMI/total_residents_dec_31,
              ...
              # calculate the number of SMI residents who had any therapy in week before assessment 
              SMI_therapy = sum(SMI_therapy, na.rm = TRUE), 
              #calculate the percent of SMI residents who had therapy
              percent_SMI_therapy = SMI_therapy/CALC_SMI,
  ...
  if (year %in% c(2013:2016)) {
    ...
    cms_list <- read_csv('for publication/CMS_list/NH_ProviderInfo_2016.csv') %>% 
  ...
  if (year == 2017){
    CMS_file <- paste0('for publication/CMS_list/NH_ProviderInfo_',year,'.csv')
  ...
  write_csv(all_residents_SMI_totals_CMS_list, paste0('final_processed_snapshot_facilities/', year, '_CA_all_facilities_snapshot.csv'))
}
